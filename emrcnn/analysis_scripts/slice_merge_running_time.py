...
# Date: 01/20/2022
# Script for testing running time of AHC and BS methods
################################################################################################
...
def slice_merge_without_ensemble_fusion(data_name, method):
    ...
    if method == "AHC":
        ...
        for i, ensemble_name in enumerate(ensemble_names):
            ...
            slice_merge_AHC(data_name, src_dir, dest_dir, config.k_list, config.voxel_thresh, config.outlier_thresh)
...
if __name__ == "__main__":
    ...
    start = time.time()
    slice_merge_ensembled_model(data_name, 'AHC')  # merge ensembled model
    # slice_merge_ensembled_model(data_name, 'layercake')  # merge ensembled model
    print('slice_merge time: ', (time.time() - start)/16) 
